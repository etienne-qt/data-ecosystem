...
2. For each report you find, **create a structured summary** in `insights/reports-external/{id}.md` following the template below.

3. **Update the registry** at `docs/reports-registry.md` — append one row per report.

4. **Update the series tracker** at `docs/reports-series.md` — group editions of recurring reports.

5. **Extract insights** and append them to:
   - `data/insights-index.csv` (one row per insight)
   - The relevant `insights/by-topic/{topic}.md` files (add to "Key Themes" and "All Insights" table)
   - The relevant `insights/by-geography/{geo}.md` files
...
1. Run `cat docs/reports-registry.md` to show the populated registry
2. Run `wc -l data/insights-index.csv` to show insight count
...
Create a file at `insights/internal/{id}.md` using this template:
...
After creating a new `internal/` card, add it to the `Internal Analyses` section of `docs/insights-legacy-index.md` and update the `updated:` date in the frontmatter.
