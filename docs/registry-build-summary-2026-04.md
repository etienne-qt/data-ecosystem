...
**Row count:** 7,607
**Build script:** `pipelines/transforms/registry/80_consolidated_startup_registry.sql`
...
- Seeded the **8 domain near-miss matches** into `REF.MANUAL_REVIEW_DECISIONS` via `pipelines/utils/sql/shared/71_seed_match_whitelist.sql`.
...
The registry is rebuilt by running `pipelines/transforms/registry/80_consolidated_startup_registry.sql` in Snowsight. Prerequisites:

- `REF` schema bootstrapped (`pipelines/utils/sql/shared/70_manual_review_tables.sql` run at least once)
...
Run `pipelines/validation/diagnostics/pipeline_row_counts.sql` and feed the outputs back into `docs/pipeline-overview.md` whenever stakeholder-facing numbers need to be updated.

### Investigating a new question

Write a focused diagnostic in `pipelines/validation/diagnostics/Qn_<question>.sql`, run in Snowsight, save CSVs, analyze. Keep diagnostics read-only and section-by-section so each answer is its own result grid. Follow the Q1–Q4 pattern.
