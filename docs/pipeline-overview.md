...
Refresh waypoints via `pipelines/validation/diagnostics/pipeline_row_counts.sql` and `Q3_startup_filter_and_lifecycle.sql`.

### Manual review (REF)

Empty as of this run — review queue infrastructure is in place but no decisions have been uploaded yet.

Refresh all numbers by running `pipelines/validation/diagnostics/pipeline_row_counts.sql` (next section).
...
**How to tune:** thresholds live in `pipelines/validation/diagnostics/Q3_startup_filter_and_lifecycle.sql`. Edit the `>` and `>=` values in the `_Q3_STARTUPS` CREATE statement; no other file needs to change.
...
Run `pipelines/validation/diagnostics/pipeline_row_counts.sql` in Snowsight. It hits every waypoint table with a single-column `COUNT(*)` query — cheap. Output is one CSV per stage, matches the waypoints table above. Drop the CSVs back to the assistant to regenerate the diagram annotations.
