...
The primary quality rating for startups, implemented in both Python (`pipelines/utils/ecosystem/processing/classifier.py`) and SQL (`pipelines/utils/sql/shared/30_udf_startup_classifier_v5.sql`).
...
Used for the `has_tech_indication` flag in the classifier and for keyword-based technology enrichment in Snowflake (`pipelines/transforms/silver/82_tech_enrichment.sql`).
...
Keyword-based industry classification via Snowflake (`pipelines/transforms/silver/81_industry_enrichment.sql`).
...
Implemented in `pipelines/transforms/silver/80_geo_enrichment.sql` using a reference mapping table `REF.CITY_REGION_MAPPING`.
...
The `CLEAN_CITY_KEY` function (`pipelines/utils/sql/shared/50_udf_city_mapping.sql`):
...
Determines whether a startup is currently active, mature, or historical. Implemented in `pipelines/transforms/silver/70_startup_lifecycle_silver.sql`.
...
Heuristic for whether a company is still operating. Implemented in `pipelines/transforms/silver/40_merge_activity_status_silver.sql`.
...
HTTP check results for company websites. Implemented in `pipelines/transforms/silver/60_drm_website_status.sql`.
