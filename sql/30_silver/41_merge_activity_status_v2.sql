...
--
-- Keys:
--   - Canonical key is DEALROOM_ID across all Dealroom SILVER tables.
-- -----------------------------------------------------------------------------
...
base AS (
    SELECT
        ...
        /* Expected columns in DRM_COMPANY_SILVER (rename here if your names differ) */
        c.company_status,
        c.closing_date,
        c.last_funding_date,
        ...
        /* Website status (either the fixed table or the alias view) */
        w.website_status,
    ...
    LEFT JOIN SILVER.DRM_WEBSITE_STATUS_SILVER w
        ON c.dealroom_id = w.company_id
        -- If you used the view instead:
        -- ON c.dealroom_id = w.dealroom_id  (same)
